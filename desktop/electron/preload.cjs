...
const api = {
  ...
  deleteLocalMod: (modName) => ipcRenderer.invoke("delete-local-mod", modName),
  ...
  analyzeOverwrites: (opts) => ipcRenderer.invoke("analyze-overwrites", opts),
  fetchWorkshopTitles: (workshopIds) => ipcRenderer.invoke("fetch-workshop-titles", workshopIds),
